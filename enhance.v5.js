(function v5Factory(){
  S.clanPlan=Object.assign({tier:'E',brawlAttacks:5,brawlAvg:50,brawlWin:false,hammerLevel:0,missionLevel:1},S.clanPlan||{});
  ...
  async function clanV5(){
    ...
    function render(){
      ...
      const maxTickets=num(wcfg.MaxWarTicketsPerMember,5),att=Math.max(0,Math.min(maxTickets,Math.floor(num(S.clanPlan.brawlAttacks,maxTickets)))),avg=Math.max(0,Math.min(num(wcfg.MaxPointsForAttackingOpponentGuildMember,50),num(S.clanPlan.brawlAvg,50))),day6=cEff(clib.WarPointsOnDay6,S.clanTech.WarPointsOnDay6),attackBase=att*avg,attackScore=attackBase*(1+day6),winBase=num(wcfg.BrawlWinPointsReward,1000),winScore=S.clanPlan.brawlWin?winBase*(1+day6):0;
      ...
      <div class="panel"><h2>6일차 전면전 계획</h2><div class="grid g4"><label class="field"><span>사용 공격권 (최대 ${maxTickets})</span><input id="batt" type="number" min="0" max="${maxTickets}" value="${att}"></label><label class="field"><span>공격 1회 예상 기본점수 (최대 ${wcfg.MaxPointsForAttackingOpponentGuildMember})</span><input id="bavg" type="number" min="0" max="${wcfg.MaxPointsForAttackingOpponentGuildMember}" value="${avg}"></label><label class="field"><span>전면전 승리</span><select id="bwin"><option value="0" ${!S.clanPlan.brawlWin?'selected':''}>아니오</option><option value="1" ${S.clanPlan.brawlWin?'selected':''}>예</option></select></label><div class="metric"><small>6일차 기술</small><b>+${fmt(day6*100)}%</b></div></div><div class="grid g4" style="margin-top:10px"><div class="metric"><small>공격 기본점수</small><b>${fmt(attackBase)}</b></div><div class="metric"><small>기술 적용 공격점수</small><b class="sum">${fmt(attackScore)}</b></div><div class="metric"><small>Brawl 승리 보너스</small><b>${S.clanPlan.brawlWin?fmt(winScore):'미적용'}</b><div class="muted small">기본 ${fmt(winBase)}</div></div><div class="metric"><small>공격+승리 보너스</small><b class="sum">${fmt(attackScore+winScore)}</b></div></div><div class="notice" style="margin-top:10px">2.9.0 설정: 개인당 공격권 최대 ${maxTickets}개 · 공격 1회 최대 ${wcfg.MaxPointsForAttackingOpponentGuildMember}점 · Brawl 승리 기본보너스 ${wcfg.BrawlWinPointsReward}점. WarPointsOnDay6는 WarPoints 대상의 OnGuildWarDay6 보너스라 6일차 점수에 적용합니다.</div></div>`;
      $('#ctier').onclick=ev=>{const b=ev.target.closest('[data-tier]');if(b){S.clanPlan.tier=b.dataset.tier;save();clanV5()}};
      $('#batt').oninput=ev=>{S.clanPlan.brawlAttacks=num(ev.target.value);save();render()};$('#bavg').oninput=ev=>{S.clanPlan.brawlAvg=num(ev.target.value);save();render()};$('#bwin').onchange=ev=>{S.clanPlan.brawlWin=ev.target.value==='1';save();render()}
    }
    render()
  }
