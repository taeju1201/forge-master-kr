(function v4Factory(){
  ...
  async function pvpV4(){
    ...
    function render(){
      const x=S.pvp,myD=x.useProfile?me.expectedHit:num(x.myD,me.expectedHit),myH=x.useProfile?me.health:num(x.myH,me.health),myA=x.useProfile?me.interval:num(x.myA,me.interval),enHit=num(x.enD,100)*(1+num(x.enCrit)/100*.2)*(1-num(me.blockChance)),myHit=myD*(1-num(x.enBlock)/100),myDps=myHit/Math.max(.01,myA),enDps=enHit/Math.max(.01,num(x.enA,1.5)),kill=num(x.enH,800)/Math.max(.0001,myDps),die=myH/Math.max(.0001,enDps);
  ...
  async function fairyV4(){
    ...
    <div class="cards">${['Lora','Mira','Tira'].map(n=>{const f=FAIRY[n],sel=S.fairy.name===n,sec=S.loadout.secondary,req=num(sec[f.required]),steps=Math.floor(Math.abs(req/100)/f.divider+1e-9),raw=steps*f.step*lv;return `<div class="card"><img class="fairyArt" src="${TEX}/FairyIcon${n}.png"><h2>${n}</h2><div class="small">${f.need} ${fmt(f.divider*100)}%마다 ${f.gain} +${fmt(f.step*lv*100)}%</div><div class="muted small">프로필 ${f.need}: ${fmt(req)}% · 변환 단계 ${steps}</div><div class="${sel?'sum':''}">예상 원시 증가 +${fmt(raw*100)}% · 상한 ${fmt(f.cap*100)}%</div></div>`}).join('')}</div>
