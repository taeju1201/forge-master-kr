(function enhanceFactory(){
  ...
  async function techTreeV2(){
    ...
    function render(){
      ...
      <div class="media"><div class="sprite" style="${spriteStyle(`${TEX}/TechTreeIcons.png`,(n.Id%10),8,128,60)}"></div><div><b>${esc(TECH_KO[n.Type]||n.Type)}</b><div class="muted small">${TREE_KO[tree]} · ${n.Tier+1}티어 · Lv.${lv}/${mx}</div><div>${effectText(n.Type,v)}</div></div></div>
  ...
  async function skinsV2(){
    ...
    function skinSprite(s){let idx=map?.skins?.mapping?.[s.SkinId.Type+'_'+s.SkinId.Idx];return idx==null?'':spriteStyle(`${TEX}/${map.skins.texture}`,idx,map.skins.grid.columns,map.skins.sprite_size.width,72)}
