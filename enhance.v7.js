(function v7Factory(){
  ...
  function skinStyle(D,type,idx){const i=D.map?.skins?.mapping?.[type+'_'+idx];if(i==null)return'';const cols=D.map.skins.grid.columns||8,rows=D.map.skins.grid.rows||8,col=i%cols,row=Math.floor(i/cols);return `background-image:url('${GTEX}/${D.map.skins.texture}');background-size:${cols*100}% ${rows*100}%;background-position:${cols>1?col*100/(cols-1):0}% ${rows>1?row*100/(rows-1):0}%;background-repeat:no-repeat;background-color:#0a0f16;`}
  function setInfo(D){
    const counts={};
